...
const PostInteraction = ({ username, count, isLiked, isReposted, isSaved, postId }: { count: { likes: number, rePostedBy: number, comments: number }, isLiked: boolean, isReposted: boolean, isSaved: boolean, postId: string, username: string }) => {

    const { user } = useUser()

    const [state, setState] = useState({
        likes: count.likes,
        rePostedBy: count.rePostedBy,
        isLiked: isLiked,
    ...
    const rePostAction = async () => {
        ...
        setState(prev => {
            return {
                ...prev,
                rePostedBy: prev.isReposted ? prev.rePostedBy - 1 : prev.rePostedBy + 1,
            ...
            <div className=" flex items-center justify-between flex-1">
                ...
                <form action={rePostAction}  >
                    <button className="flex items-center gap-2 cursor-pointer group">
                        ...
                        <span className={`${optimisticCount.isReposted ? "text-iconGreen" : "text-textGray"}  group-hover:text-iconGreen text-sm`}>{optimisticCount.rePostedBy}</span>
                    </button>
                </form >
                {/* LIKE */}
